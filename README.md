...
- Bundled overlays ship with the build (served from `kml-library/`) and are drawn on the map
  automatically on first run — no upload step
...
- Linux packages carry the bundled overlays in `resources/kml-seed`; they are seeded into the
  library on first run and drawn automatically. Windows installers omit them to stay small and
...
- A point visualizer (**Points** tab) showing one sample in its own neighbourhood: an
  equal-aspect local plan view and elevation profile, the legs either side with interval,
  geodesic distance, implied speed, elevation change and bearing, full provenance and channel
  values, and a whole-track quality strip to jump between samples.
- Synchronized transient data cursor across chart, map, table, points and 3D.
- A selection badge in each view that jumps to the selected samples when clicked — fit, zoom,
  scroll, or camera pan depending on the view — and discards the selection only through its ×.
...
Every repair that changes the track is shown before it is applied. The original and the
proposed repair are drawn on one frame — plan view, profile, or both — with the samples the
repair added, removed, moved or retimed marked, alongside the operation's summary, warnings and
point counts. Nothing is applied until **Accept**; Escape, clicking away and closing all
**Revert**, so making no choice leaves the track untouched. A repair that only adds computed
channels has nothing to overlay and applies without a gate, and a "preview repairs" toggle in
the Transform toolbar turns the gate off for batch work.
