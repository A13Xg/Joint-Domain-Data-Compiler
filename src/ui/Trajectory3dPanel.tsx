...
export function Trajectory3dPanel({ dataset }: { dataset: Dataset }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      path(context, projected, colorChannelId, geometry.colorRange, indexRange)
  ...
  }, [geometry, camera, projection, showGrid, showCurtain, showPoints, colorChannelId, indexRange, pointIndex, hoverIndex, playback])
  ...
  return <div className="analysis-panel trajectory-panel">
    <div className="analysis-toolbar trajectory-toolbar">
      ...
      <button type="button" onClick={() => setCamera(DEFAULT_CAMERA)}>Reset camera</button><button type="button" onClick={() => setCamera((current) => ({ ...current, zoom: 1, panX: 0, panY: 0 }))}>Fit trajectory</button>
