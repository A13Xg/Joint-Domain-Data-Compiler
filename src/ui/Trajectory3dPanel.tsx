...
export function Trajectory3dPanel({ dataset }: { dataset: Dataset }) {
  ...
  const [gapThresholdSeconds, setGapThresholdSeconds] = useState(3)
  ...
  const { pointIndex, hoverIndex, indexRange, selectPoint, setHoverIndex, clearSelection, clearRange, clearHover } = usePointSelection(dataset.points)
  const geometry = useMemo(() => buildTrajectory3dGeometry(dataset.points, { altitudeExaggeration, maxPoints: 20_000, colorChannelId: colorChannelId || undefined }), [dataset.points, altitudeExaggeration, colorChannelId])
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      path(context, projected, colorChannelId, geometry.colorRange, indexRange, gapThresholdSeconds * 1000)
  ...
  }, [geometry, camera, projection, showGrid, showCurtain, showPoints, colorChannelId, indexRange, pointIndex, hoverIndex, playback, gapThresholdSeconds])
  ...
  return <div className="analysis-panel trajectory-panel">
    <div className="analysis-toolbar trajectory-toolbar">
      <label className="num-field"><span>altitude exaggeration</span><input type="number" min={0.1} max={100} step={0.5} value={altitudeExaggeration} onChange={(event) => setAltitudeExaggeration(clamp(Number(event.target.value) || 1, 0.1, 100))} /></label>
      ...
      <label className="num-field"><span>projection</span><select value={projection} onChange={(event) => setProjection(event.target.value as Projection)}><option value="perspective">perspective</option><option value="orthographic">orthographic</option></select></label>
      <label className="num-field"><span>gap split (s)</span><input type="number" min={0} step={1} value={gapThresholdSeconds} onChange={(event) => setGapThresholdSeconds(Math.max(0, Number(event.target.value) || 0))} /></label>
      ...
      {pointIndex !== null && <button type="button" className="chip chip-on" onClick={clearSelection}>selected #{pointIndex} ×</button>}{indexRange && <button type="button" className="chip chip-range" onClick={clearRange}>range {indexRange.start}–{indexRange.end} ×</button>}
    </div>
...
function path(context: CanvasRenderingContext2D, vertices: ScreenVertex[], channel: string, range: { min: number; max: number } | undefined, selected: { start: number; end: number } | null, gapThresholdMs: number) { for (let index = 1; index < vertices.length; index++) { const a = vertices[index - 1]!, b = vertices[index]!, gap = gapThresholdMs > 0 && b.time !== undefined && a.time !== undefined && b.time - a.time > gapThresholdMs; if (gap) continue; const inRange = selected !== null && b.sourceIndex >= selected.start && b.sourceIndex <= selected.end; context.beginPath(); context.moveTo(a.x, a.y); context.lineTo(b.x, b.y); context.strokeStyle = inRange ? '#facc15' : vertexColor(b, range, channel); context.lineWidth = inRange ? 4.5 : 2.2; context.stroke() } }
