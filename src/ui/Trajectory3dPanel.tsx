...
export function Trajectory3dPanel({ dataset, workspace, onWorkspaceChange }: { dataset: Dataset; workspace: WorkspaceState['scene3d']; onWorkspaceChange: (next: WorkspaceState['scene3d']) => void }) {
  ...
  }, [geometry, camera, projection, showGrid, showCurtain, showPoints, colorChannelId, indexRange, pointIndex, hoverIndex, playback, pathBreakIndices, jumpIndices])
  ...
  return <div className="analysis-panel trajectory-panel">
    <div className="analysis-toolbar trajectory-toolbar">
      ...
      {pointIndex !== null && <button type="button" className="chip chip-on" onClick={clearPointSelection}>selected #{pointIndex} ×</button>}{indexRange && <button type="button" className="chip chip-range" onClick={clearRangeSelection}>range {indexRange.start}–{indexRange.end} ×</button>}
    </div>
