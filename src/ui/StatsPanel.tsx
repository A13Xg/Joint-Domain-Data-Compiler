...
export function StatsPanel({ dataset }: { dataset: Dataset }) {
  ...
  return <div className="stats-panel">
    ...
    <section className="stats-block selection-controls"><h3>Selection controls</h3>
      ...
      <p className="muted small">Keyboard: ←/→ moves the synchronized cursor, Shift+←/→ extends a range, Enter selects the cursor point, Home/End jumps, Escape clears.</p>
    </section>

    <div className="stats-columns"><section className="stats-block"><h3>Data quality</h3><ul className="quality-list">{quality.map((item) => <li key={item.label} className={item.ok ? 'q-ok' : 'q-warn'}><span className="q-icon">{item.ok ? '✓' : '!'}</span><span className="q-label">{item.label}</span><span className="q-detail">{item.detail}</span></li>)}</ul>{stats.startTime !== null && <p className="muted mono small">{epochMsToIso(stats.startTime)} → {epochMsToIso(stats.endTime ?? undefined)}</p>}{stats.bounds && <p className="muted mono small">bbox [{stats.bounds.minLat.toFixed(5)}, {stats.bounds.minLon.toFixed(5)}] → [{stats.bounds.maxLat.toFixed(5)}, {stats.bounds.maxLon mono small">bbox [{stats.bounds.minLat.toFixed(5)}, {stats.bounds.minLon.toFixed(5)}] → [{stats.bounds.maxLat.toFixed(5)}, {stats.bounds.maxLon.toFixed(5)}]</p>}{dataset.warnings.length > 0 && <div className="stats-warnings">{dataset.warnings.map((warning, index) => <p key={index} className="warn-line small">⚠ {warning}</p>)}</div>}</section>
