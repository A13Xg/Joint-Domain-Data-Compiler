...
interface Props {
  ...
  /** Runs the drop-outliers operation with the config this scan graded against. */
  onDropOutliers: () => void
}
...
export function TrackHealthPanel({ dataset, onDrillDown, onDropOutliers }: Props) {
  ...
  return (
    <div className="track-health-panel">
      ...
      {scanning && !scan.report && (
        <div className="health-checks health-checks-skeleton" aria-hidden="true">
          {Array.from({ length: 6 }, (_, index) => (
            <div className="health-check" key={index}>
