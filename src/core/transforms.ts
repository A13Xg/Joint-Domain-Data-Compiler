...
import { clonePoint, collectChannels, haversineMeters, inferChannelDefinitions, isValidLat, isValidLon, type Dataset, type TrackPoint } from './model'
 ...
 *
 * The final point is always kept, even when its index is not a multiple of
 * the factor. Dropping it would shorten the track's time and distance extent
 * by up to `factor - 1` samples — a silent change to the data's stated span,
 * which downsampling is not licensed to make.
 */
export function decimate(points: TrackPoint[], factor: number): TransformResult {
  const f = Math.max(1, Math.floor(factor))
  const last = points.length - 1
  const out = clone(points.filter((_, i) => i % f === 0 || i === last))
  return { points: out, summary: `Decimated to every ${ordinal(f)} point (${out.length} kept)` }
}

/** English ordinal, so a factor of 2 reads "2nd" rather than "2th". */
 ...
 * amplitude/phase distortion a moving-average filter introduces; unlike a
 * point-dropping outlier filter, this never changes the point count.
 * Index-window based (not time-window based), so it makes no assumption about uniform
...
/**
 * Hampel identifier over elevation: detects spikes the same way the
 * `drop-outliers` operation does, but replaces each with the local median
