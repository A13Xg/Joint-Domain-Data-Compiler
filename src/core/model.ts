...
export interface Dataset {
  ...
  sourceBytes?: number
  /** Records the parser rejected before import, keyed by reason. */
...
export interface ParseResult {
  ...
  coordinateSystem?: string
  /**
   * Records the parser rejected, keyed by reason.
   *
   * Parsers already stringify some of these into `warnings`, and GPX and KML
   * dropped points with no record at all. A structured count is what lets the
   * UI say "5 of 105 records were dropped" without scraping English prose,
   * and what makes a silent drop impossible to add without noticing.
   */
  droppedCounts?: Record<string, number>
}
...
export function collectChannels(points: readonly TrackPoint[]): string[] {
