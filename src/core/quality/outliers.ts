...
export function detectOutliers(points: readonly TrackPoint[], config: OutlierConfig): OutlierResult {
  ...
  for (let slot = 0; slot < eligible.length; slot++) {
    ...
    const scores: [OutlierChannel, number][] = [['position', Math.abs(positionResiduals[slot] ?? 0) / positionScale]]
    const elevationResidual = elevationResiduals[slot]
    if (elevationResidual !== undefined) scores.push(['elevation', Math.abs(elevationResidual) / elevationScale])
    const speedResidual = speedResiduals[slot]
    if (speedResidual !== undefined) scores.push(['speed', Math.abs(speedResidual) / speedScale])

    let channel: OutlierChannel = 'position'
    let best = 0
