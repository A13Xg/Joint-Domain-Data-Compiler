...
export const speedEnvelopeCheck: TrackHealthCheckDefinition = {
  ...
  run: (points, _dataset, config, shared) => {
    ...
    const { minSpeedMps, maxSpeedMps, maxViolationFraction } = config.speedEnvelope
    const speeds = derivePointSpeeds(points)
    const violations: TrackHealthFlag[] = []
    let sampled = 0

    for (let index = window.startIndex; index <= window.endIndex && index < points.length; index++) {
    ...
    if (sampled === 0) return notApplicable('speed-envelope', 'Speed Envelope', speedEnvelopeCheck.weight, 'No speed samples inside the movement window', 'charts')
    ...
    return {
      ...
      flags,
      preferredTab: 'charts',
      measurements: { sampled, violations: violations.length, violationFraction },
    }
  },
}
