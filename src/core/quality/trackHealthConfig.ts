...
export interface TrackHealthConfig {
  ...
  speedEnvelope: {
    ...
    /**
     * Leading share of the file this check ignores outright, on top of the movement
     * window. Recordings routinely start with the aircraft parked and powered up, which
     * sits below the 10 kt floor for as long as the crew takes to get moving; that is
     * normal operation, not a data defect, and it should not spend this check's violation
     * budget. Applies to the speed envelope only — every other check still sees the whole
     * track, and nothing is removed from the dataset.
     */
    leadInIgnoreFraction: number
  }
  timeOrderSpan: {
    minSpanMs: number
    maxSpanMs: number
