...
function persistedHistories(archive: ProjectArchive): Record<string, PersistedHistory> {
  ...
  return Object.fromEntries(Object.entries(archive.histories).map(([datasetId, history]) => {
    ...
    // past[0] is pinned to the original import, so once the snapshot limit is
    // reached the step from past[0] to past[1] is no longer a single
    // operation. makeDelta finds no matching record for that jump and falls
    // back to a structural patch, which round-trips correctly but costs one
    // extra full copy of the points on disk. That only happens past 50
    // operations, and it is the price of keeping the original restorable.
...
function validateDataset(value: unknown): asserts value is Dataset {
  ...
  if (value.droppedCounts !== undefined) {
    if (!isRecord(value.droppedCounts)) throw new Error(`Dataset ${value.id} droppedCounts must be an object`)
    for (const [reason, count] of Object.entries(value.droppedCounts)) {
      if (isUnsafeObjectKey(reason)) throw new Error(`Dataset ${value.id} droppedCounts key is unsafe`)
      requireFinite(count, `Dataset ${value.id} droppedCounts.${reason}`)
      if (count < 0) throw new Error(`Dataset ${value.id} droppedCounts.${reason} must not be negative`)
    }
  }
