...
const filtered = hampelFilterElevation(elevations, 4, 11)
check('Rolling MAD rewrites a local elevation spike', filtered.points[10]!.ele !== 1000, `${filtered.points[10]!.ele} m`)
check('Rolling MAD preserves the point count', filtered.points.length === 21, `${filtered.points.length} points`)
