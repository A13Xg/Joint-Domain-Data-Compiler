...
import { appendHistorySnapshot, MAX_HISTORY_SNAPSHOTS } from '../src/state/history'
...
// A history pruned past MAX_HISTORY_SNAPSHOTS has a non-adjacent step from
// past[0] (the pinned original) to past[1]. No operation record matches that
// jump, so it must serialize as a structural patch and still round-trip.
ensureBuiltinOperationsRegistered()
let prunedPast: Dataset[] = []
let prunedCurrent = dataset
for (let step = 0; step < MAX_HISTORY_SNAPSHOTS + 5; step++) {
  prunedPast = appendHistorySnapshot(prunedPast, prunedCurrent)
  prunedCurrent = executeOperation(prunedCurrent, 'offset-elevation', { meters: 1 }).dataset
}
assert.equal(prunedPast.length, MAX_HISTORY_SNAPSHOTS, 'the pruned history is at the limit')
assert.deepEqual(prunedPast[0], dataset, 'the original is still at index 0 after pruning')

const prunedArchive = createProjectArchive({
  manifest: buildProjectManifest({
    datasets: [prunedCurrent],
    activeDatasetId: dataset.id,
    activeTab: 'transform',
    selection: { ...EMPTY_WORKSPACE_SELECTION, datasetId: dataset.id },
    applicationVersion: '0.1.0',
  }),
  datasets: [prunedCurrent],
  histories: { [dataset.id]: { past: prunedPast, future: [] } },
})
const prunedRoundTrip = parseProjectArchive(serializeProjectArchive(prunedArchive))
assert.deepEqual(
  prunedRoundTrip.histories[dataset.id]?.past[0],
  dataset,
  'the original survives a project archive round-trip of a pruned history',
)
assert.equal(
  prunedRoundTrip.histories[dataset.id]?.past.length,
  MAX_HISTORY_SNAPSHOTS,
  'every retained snapshot survives the round-trip',
)
