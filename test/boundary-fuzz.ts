...
const VALID_OPERATION_PARAMS: Record<string, unknown[]> = {
  ...
  'sort-by-time': [undefined, {}],
  'swap-lat-lon': [undefined, {}],
  'drop-invalid': [undefined, {}],
  'dejitter-timestamps': [
    { duplicatePolicy: 'nudge', epsilonMs: 1 },
    { duplicatePolicy: 'drop', epsilonMs: 0.5 },
    { duplicatePolicy: 'average', epsilonMs: 1000 },
  ],
  'clip-time-range': [
    { startMs: 0, endMs: 1000, untimedPolicy: 'keep' },
  ...
  'elevation-filter': [
    ...
    { mode: 'ema', alpha: 0.25 },
    { mode: 'hampel', window: 11, sigmaThreshold: 3 },
  ],
  smooth: [{ window: 5, coords: true, elevation: false }, { window: 2, coords: false, elevation: true }],
  'reduce-points': [
    { mode: 'dedupe', toleranceMeters: 0 },
    { mode: 'decimate', factor: 2 },
    { mode: 'simplify', epsilonMeters: 5 },
  ],
  'round-precision': [{ coordinateDecimals: 6 }, { coordinateDecimals: 5, elevationDecimals: 2, channelDecimals: 3 }],
  'fill-gaps': [
...
const TOLERATED_BY_OPERATION: Record<string, string[]> = {
  ...
  // Parameterless operations, like standard-kinematics: `undefined` and `{}`
  // are both the recorded form of "no parameters".
  'sort-by-time': ['undefined', 'empty object'],
  'swap-lat-lon': ['undefined', 'empty object'],
  'drop-invalid': ['undefined', 'empty object'],
  'dejitter-timestamps': [],
  'clip-time-range': [],
  'drop-outliers': [],
  'elevation-filter': [],
