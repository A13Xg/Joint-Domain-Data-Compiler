...
// The density fields follow the same rule as every other map field: malformed
// input defaults rather than rejecting the whole project load.
assert.equal(normalizeWorkspaceState({ map: { showDensity: 'yes' } }, datasetIds).map.showDensity, false)
assert.equal(normalizeWorkspaceState({ map: { showDensity: true } }, datasetIds).map.showDensity, true)
assert.equal(normalizeWorkspaceState({ map: { densityCellMeters: 0 } }, datasetIds).map.densityCellMeters, DEFAULT_WORKSPACE_STATE.map.densityCellMeters)
assert.equal(normalizeWorkspaceState({ map: { densityCellMeters: 'wide' } }, datasetIds).map.densityCellMeters, DEFAULT_WORKSPACE_STATE.map.densityCellMeters)
assert.equal(normalizeWorkspaceState({ map: { densityCellMeters: 750 } }, datasetIds).map.densityCellMeters, 750)
