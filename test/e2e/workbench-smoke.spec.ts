...
const csvFixture = resolve('test/fixtures/real-usgs.csv')
...
test('verified transform history replays from its retained source snapshot', async ({ page }) => {
  ...
  // Repairs are previewed before they are applied; Accept is what commits one.
...
test('named recipes can be captured, replayed, deleted, and persisted', async ({ page }) => {
  ...
  await expect(confirmDialog).toContainText('Ten meter offset')
...
test('primary local-first workflow: import, inspect, transform, save/open, and export', async ({ page }) => {
  ...
  await expect(page.getByText('No datasets loaded')).toBeVisible()
  ...
  await expect(page.locator('.point-detail h4')).toHaveText('Sample 2')
  await expect(page.locator('.point-plan svg')).toBeVisible()
  await expect(page.locator('.point-deltas')).toContainText('to next')
  // Stepping here has to move the shared selection, not a local copy of it.
  await page.getByRole('button', { name: 'Next point' }).click()
  await expect(page.locator('.point-detail h4')).toHaveText('Sample 3')
  await page.getByRole('button', { name: 'Table', exact: true }).click()
  await expect(page.getByRole('button', { name: 'selected #3' })).toBeVisible()
  await page.locator('.grid-row').nth(2).click()
