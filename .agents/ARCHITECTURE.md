...
| `mapOverlays.ts` | KML/KMZ overlay library state (`bundled` / `library` / `project`). |
...
`test/<format>-parser.ts` and a `file-test/` fixture.

**Add a transform:** pure function in `transforms.ts` → wrap as an `OperationDefinition`
in `operations/` → register → add a card in `TransformPanel.tsx`.
